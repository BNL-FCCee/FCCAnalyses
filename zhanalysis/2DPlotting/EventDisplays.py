...
def main():
    ...
    nevents = 5
    e_cut = 0
    inFile = "./2DPlotting/antikt/stage2/output.root"
...
#make an array for each column with their respective values for nevents
    for idx in range (nevents):
        #this gives H_phi event info idx
...
#  = {mZ1:.1f}$ GeV, $m(Z_2) = {mZ2:.1f}$ GeV, $m(H) = {higgsrecomass:.1f}$ GeV",
        ...
        ax.set_title("Z(cc)H(bb) events, after selections, Anti-kt R=0.4", loc='left',
                fontdict={'fontweight':'bold'}, y=1.03)
        plt.savefig(f"./2DPlotting/antikt/outputs/Display_{idx}.png", dpi=500)
        plt.close()
    pass
