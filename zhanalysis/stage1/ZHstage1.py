...
JobName = "ZHallHadAntikT_OCT24" # job named used for output directory
...
def jet_sequence(df,rad, alg, sort, ecut):
    ...
    for p_var in p_vars:
        df = df.Define("truth_H_{}".format(p_var), "FCCAnalyses::MCParticle::get_{}(truth_H)".format(p_var))

        df = df.Define("truth_C_{}".format(p_var), "FCCAnalyses::MCParticle::get_{}(truth_C)".format(p_var))
